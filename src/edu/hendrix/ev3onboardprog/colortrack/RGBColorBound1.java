...
public class RGBColorBound1 extends MinMaxBound<RGB> {
	...
	public YUVColorBound makeYUV() {
		return new YUVColorBound(inside, 
				getY(rMin, gMin, bMin), 
				getY(rMax, gMax, bMax),
				getU(rMin, gMin, bMin),
				getU(rMax, gMax, bMax),
	...
	private static int formula(int r, int g, int b, int rc, int gc, int bc, int add) {
		return ((CONVERT * (rc * r + gc * g + bc * b) + 128) >> 8) + add;
	}
	...
	public RGBColorBound1(boolean inside) {
		super(inside, 1, MAX);
	}
